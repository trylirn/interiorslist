...
export function MatchResultCard({
  ...
  const otherServices = (m.services ?? []).filter((s) => !matchedServices.includes(s)).slice(0, 3);
  const otherStyles = (m.styles ?? []).filter((s) => !matchedStyles.includes(s)).slice(0, 3);
  ...
  return (
    <article
      className={`flex h-full flex-col rounded-2xl border bg-card p-5 transition ${
    ...
    >
      ...
      {reasons.length > 0 && (
        <p className="mt-3 rounded-xl bg-brand/5 px-3 py-2 text-sm">
      ...
      {(matchedStyles.length > 0 || otherStyles.length > 0) && (
        <div className="mt-3">
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Styles</p>
      ...
      {m.specialists && <p className="mt-3 line-clamp-3 text-sm text-muted-foreground">{m.specialists}</p>}

      <div className="mt-auto pt-4">
        <Link to="/provider/$slug" params={{ slug: m.slug }} className="text-sm font-medium text-brand hover:underline">
          View full profile →
        </Link>
      </div>
    </article>

...
export function RelatedPosts({ tags }: { tags: string[] }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => listRelatedPostsForProvider({ data: { tags: tags.slice(0, 12), limit: 3 } }),
  });
    ...
    <section className="mt-12">
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-3">
        {posts.map((p: any) => (
          ...
          >
            {p.cover_url ? (
              <img src={p.cover_url} alt="" loading="lazy" className="h-32 w-full object-cover" />
            ) : (
            ...
            <div className="p-4">
              ...
              <h3 className="mt-1.5 font-display text-lg leading-snug group-hover:text-brand">{p.title}</h3>

...
function SubmitPage() {
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-12">
      <h1 className="font-display text-4xl">Submit a business</h1>
      <p className="mt-2 text-muted-foreground">Know a great Texas injector we're missing? Tell us about them.</p>
      <form onSubmit={handle} className="mt-8 space-y-4">
        <div className="space-y-1.5"><Label>Business name *</Label><Input name="businessName" required maxLength={200} /></div>
        ...
        <div className="space-y-1.5"><Label>Notes</Label><Textarea name="notes" maxLength={2000} rows={4} /></div>
        <Button type="submit" disabled={loading} className="w-full h-11">{loading ? "Submitting…" : "Submit business"}</Button>
      </form>
    </div>
  );
}
